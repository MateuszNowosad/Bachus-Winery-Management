    ...
    ? dataObject.map(currElement => {
        ...
        for (let value in values) {
          if (value < values.length - 1) {
            ...
            } else cells.push({ text: values[value] });
          }
        }
        data.push(cells);
      })
...
const PDFFromDataSet = (dataObject, dataLabels, pageSize, pageOrientation, fontSize, title) => {
  return {
    pageSize: pageSize,
    pageOrientation: pageOrientation,
    pageMargins: [40, 20, 40, 60],
    content: [
      { text: currentDate('date'), fontSize: 8, alignment: 'right', margin: [0, -10, -10, -10] },
      { text: title, alignment: 'center', fontSize: fontSize + 5, bold: true, margin: [0, 0, 0, 15] },
      {
        table: {
          dontBreakRows: true,
          headerRows: 1,
